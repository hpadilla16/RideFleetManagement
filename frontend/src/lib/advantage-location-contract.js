/**
 * The Advantage location-config REQUEST CONTRACT — the single place the panel
 * names the fields it sends to `/api/admin/integrations/advantage/locations`.
 *
 ...
 * uses to shape a location body, `readLocationBody()` in advantage.routes.js is
 * the only thing the routes use to read one, and
 * backend/src/modules/integrations/advantage/advantage.routes.test.mjs feeds this
 ...
 * import it directly.
 *
 * Identity note: an Advantage config row is keyed by the PAIR (tsdNumber, branch)
 * — the portal's `Loc` column, e.g. "61302.MCO" — not by a single id.
 */

/** Body keys the routes understand. Must match LOCATION_BODY_FIELDS in advantage.routes.js. */
export const ADVANTAGE_LOCATION_BODY_FIELDS = Object.freeze([
  'tsdNumber',
  'branch',
  'locationId',
  'enabled',
 ...
 */
export function buildAdvantageLocationBody(input = {}) {
  const body = {};
...
export function advantageLocLabel(row) {
