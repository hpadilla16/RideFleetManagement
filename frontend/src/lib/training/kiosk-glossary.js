  ...
  groups: [
    ...
    },
    {
      ...
      entries: [
        ...
        { id: 'cant-scan', labels: ['kiosk.idCantScan'], tone: 'secondary', what: 'Escalates: “A team member is on the way” — the counter is notified. Nothing escalates by itself; the guest (or you) presses it.' },
    ...
    },
    {
      ...
      entries: [
        ...
        { id: 'verify', labels: ['kiosk.assistVerifyBtn'], tone: 'staff', what: 'Runs two real rules — age and license validity. The name check is deliberately skipped here (you are holding the license). YOUR PIN DOES NOT SKIP THE OTHER TWO.' },
    ...
    },
    {
      ...
      entries: [
        ...
        { id: 'retry', labels: ['kiosk.payRetry'], tone: 'guest', what: 'Only on the sandbox/failed screen; it is not a link action. A real declined card is retried by the guest on their phone from the same QR. Pressing “Show payment code” again shows the same link for the same total.' },
        { id: 'change-extras', labels: ['kiosk.payChangeExtras'], tone: 'secondary', what: 'Back to extras. A new total mints a NEW link; the previous one leaves the screen but stays payable if the guest left it open on their phone — have them close it. Two payments landing = refund the extra, never re-charge.' },
        { id: 'simulate', labels: ['kiosk.paySimulate'], tone: 'secondary', what: 'Sandbox stamp for test environments. It is still on screen in production but refuses (“Sandbox payment isn’t enabled…”) — never press it in front of a guest.' },
    ...
    },
    {
      ...
      entries: [
        ...
        { id: 'done', labels: ['kiosk.doneKeysStaff'], tone: 'note', what: 'The “All set!” screen has no buttons: keys at the counter (or the lockbox, if your location is set to it), contract and receipt by email, a QR to photo-document the car. It resets after a 30-second countdown.' },
      ],
    },
    {
      key: 'pair',
