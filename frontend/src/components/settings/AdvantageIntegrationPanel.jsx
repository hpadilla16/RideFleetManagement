...
/**
 * AdvantageIntegrationPanel — Settings tab for the Advantage (TSD RezCentral)
 ...
 * doc/advantage-panel-mockup-2026-07-14.html (screens AD-1 / AD-2 / AD-3).
 *
 ...
 *   - NO PREPAID SURFACE. Advantage is 100% Pay on Arrival (confirmed with
 *     Advantage 2026-07-14) — there is nothing to show, so nothing is shown.
 ...
 * the locale files).
 *
 * Backend contract (mounted at /api/admin/integrations/advantage):
 ...
 *   DELETE /locations/:id
 *
 * The location bodies above are built ONLY through buildAdvantageLocationBody()
 * so the panel and advantage.routes.js cannot drift on field names — see
 * lib/advantage-location-contract.js (the beta.301 regression).
 */

import Link from 'next/link';
import { useEffect, useMemo, useState } from 'react';
import { api } from '../../lib/client';
import { buildAdvantageLocationBody, advantageLocLabel } from '../../lib/advantage-location-contract';
...
export function AdvantageIntegrationPanel({ token, me, isSuper, isAdmin, tenantName, activeSettingsTenantId, scopedSettingsPath, onPageMsg }) {
  ...
  const reload = async () => {
    ...
    try {
      const [s, r, l, rl] = await Promise.all([
        api(scoped('/api/admin/integrations/advantage/status'), { bypassCache: true }, token).catch(() => null),
        api(scoped('/api/admin/integrations/advantage/runs?limit=7'), { bypassCache: true }, token).catch(() => ({ runs: [] })),
        api(scoped('/api/admin/integrations/advantage/locations'), { bypassCache: true }, token).catch(() => ({ rows: [] })),
  ...
  const handleToggleMaster = async () => {
    ...
    try {
      const res = await api(
        scoped('/api/admin/integrations/advantage/enabled'),
  ...
  const handleSaveCreds = async (e) => {
    ...
    try {
      const res = await api(
        scoped('/api/admin/integrations/advantage/credentials'),
  ...
  const handleTestAuth = async () => {
    ...
    try {
      const res = await api(
        scoped('/api/admin/integrations/advantage/test-auth'),
        { method: 'POST' },
        token
      );
  ...
  const handleForceRelogin = async () => {
    ...
    try {
      const res = await api(
        scoped('/api/admin/integrations/advantage/force-relogin'),
        { method: 'POST' },
        token
      );
  ...
  const handleRunNow = async () => {
    ...
    try {
      const res = await api(
        scoped('/api/admin/integrations/advantage/run-now'),
        { method: 'POST' },
        token
      );
  ...
  // Every body below goes through buildAdvantageLocationBody — the panel never
  // hand-writes a field name (beta.301).
  const locLabel = (row) => row?.loc || advantageLocLabel(row) || '';
  ...
  const handleAddConfig = async (e) => {
    ...
    try {
      const res = await api(
        scoped('/api/admin/integrations/advantage/locations'),
        {
          method: 'POST',
          body: JSON.stringify(buildAdvantageLocationBody({
  ...
  const patchConfig = async (id, input) => {
    try {
      const res = await api(
        scoped(`/api/admin/integrations/advantage/locations/${id}`),
        { method: 'PUT', body: JSON.stringify(buildAdvantageLocationBody(input)) },
        token
      );
  ...
  const toggleConfig = async (row) => {
    try {
      const res = await api(
        scoped(`/api/admin/integrations/advantage/locations/${row.id}/toggle`),
  ...
  const removeConfig = async (row) => {
    ...
    try {
      const res = await api(
        scoped(`/api/admin/integrations/advantage/locations/${row.id}`),
        { method: 'DELETE' },
        token
      );
      if (res?.ok) await reload();
  ...
  return (
    <div className="stack" style={{ gap: 20 }}>
      ...
      <section className="glass card section-card">
        <div className="row-between" style={{ alignItems: 'flex-start' }}>
          <div style={{ display: 'grid', gap: 6 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <h2 style={{ margin: 0 }}>
                Advantage <span style={{ fontWeight: 600, color: '#6f668f', fontSize: 15 }}>(TSD RezCentral)</span>
              </h2>
            ...
            <p className="ui-muted">
              Imports reservations from the Advantage back-office (<code>rezcentral.tsdasp.net</code>) into Ride.{' '}
              ...
              <span style={{ background: 'rgba(135,82,254,.15)', color: '#5a2fca', border: '1px solid rgba(135,82,254,.22)', padding: '1px 7px', borderRadius: 999, fontSize: 11, fontWeight: 600, display: 'inline-block' }}>
                Franchise import · ADV-
              </span>{' '}
              and go through the same review tray as Economy, NU and Flexways. Every Advantage booking is{' '}
        ...
        {!status?.integrationEnabled ? (
          <div className="ui-muted" style={{ fontSize: 12 }}>
            The autonomous scheduler is currently <strong>off</strong> platform-wide (ADVANTAGE_INTEGRATION_ENABLED). You can configure
      ...
      <section className="glass card section-card">
        ...
        <p className="ui-muted">
          The Advantage back-office (<code>rezcentral.tsdasp.net/WebRezClient</code>) uses a <strong>username + password</strong> login.
      ...
      <section className="glass card section-card">
        <div className="row-between" style={{ alignItems: 'flex-start' }}>
          <div style={{ display: 'grid', gap: 4 }}>
            <h3 style={{ margin: 0 }}>Advantage locations</h3>
            <p className="ui-muted">
              Advantage keys its reports by <strong>TSD account number</strong> + <strong>branch</strong>. Map each pair to a Ride
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>
              {locations.length === 0 ? (
                <tr><td colSpan={6} style={{ padding: 12, color: '#6b7280' }}>No Advantage locations configured yet. Add one below.</td></tr>
              ) : locations.map((row) => (
        ...
        <div className="surface-note" style={NOTE_STYLE}>
          ...
          the Advantage side only need a row here — no code changes.
        </div>
      </section>
      ...
      <section className="glass card section-card">
        ...
        <div className="surface-note" style={{ ...NOTE_STYLE, background: 'linear-gradient(180deg, rgba(245,158,11,.10), rgba(245,158,11,.04))', border: '1px solid rgba(245,158,11,.22)' }}>
          📭 <strong>About half of Advantage bookings arrive with no customer email</strong> — that is how the source data comes, not a
      ...
      <section className="glass card section-card" style={{ borderLeft: '4px solid #f59e0b' }}>
        ...
        <div className="surface-note" style={NOTE_STYLE}>
          ...
          labeled <strong>“Pending franchise imports — Advantage”</strong>. It is not duplicated here.
        </div>
      </section>
    </div>
  );
...
export default AdvantageIntegrationPanel;
