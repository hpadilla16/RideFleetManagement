...
import { KioskFrame, Heading, Btn, Callout, Card, Line, K, W } from './KioskFrame';
...
export function StaffPin() {
  ...
  return (
    <KioskFrame step={2} help={false} label={t('kiosk.assistTitle', 'Staff assistance')}>
      ...
      <Line x={46} y={139} text="Ana Rivera" size={13} color={K.ink} />
      ...
      <Btn x={186} y={218} w={126} label={t('kiosk.assistCancel', 'Cancel — back to the guest')} tone="secondary" />
      <Keypad x={400} y={110} />
...
function GrantChip({ y = 58 }) {
  ...
  return (
    <g>
      ...
      <Line x={494} y={y + 17} anchor="middle" text={t('kiosk.assistGrantChip', '{{name}} — {{time}} left', { name: 'Ana Rivera', time: '9:58' })} size={11} weight={600} color={K.mintInk} max={36} />
    </g>
  );
}
  ...
  return (
    <KioskFrame step={2} help={false} label={t('kiosk.assistFormTitle', 'Enter the guest’s ID by hand')}>
      ...
      {[
        ...
        <g key={l}>
          <Line x={32} y={118 + i * 40} text={l} size={10} weight={700} color={K.muted} />
      ...
      <Card x={310} y={110} w={140} h={100}>
        <Line x={380} y={150} anchor="middle" text={t('kiosk.assistFront', 'FRONT of the licence')} size={10} weight={700} color={K.muted} max={22} />
        <Line x={380} y={170} anchor="middle" text={t('kiosk.assistTapCapture', 'tap to capture')} size={10} color={K.muted} />
      </Card>
      <Card x={462} y={110} w={140} h={100}>
        <Line x={532} y={150} anchor="middle" text={t('kiosk.assistBack', 'BACK of the licence')} size={10} weight={700} color={K.muted} max={22} />
        <Line x={532} y={170} anchor="middle" text={t('kiosk.assistTapCapture', 'tap to capture')} size={10} color={K.muted} />
      </Card>
      <Callout n={1} x={456} y={110} />
      ...
      <Line x={310} y={288} text={t('kiosk.assistBothPhotos', 'Photos of the FRONT and the BACK of the licence are required.')} size={10.5} color={K.badInk} max={62} />
    </KioskFrame>
  );
}
    ...
    <KioskFrame step={2} help={false} label={t('kiosk.assistVerifyBtn', 'Verify and continue')}>
      ...
      <Card y={116} h={104}>
        {[
          ...
          [t('kiosk.checkAge', 'Age requirement'), false],
          [t('kiosk.checkExpiry', 'Licence valid during your rental'), true],
        ].map(([label, ok], i) => (
          <g key={label}>
            <circle cx="56" cy={142 + i * 28} r="8" fill={ok ? K.mint : K.bad} />
            <Line x={56} y={146 + i * 28} anchor="middle" text={ok ? '✓' : '✕'} size={10} weight={700} color={ok ? K.mintInk : K.badInk} />
            <Line x={76} y={146 + i * 28} text={label} size={12} color={K.ink} max={70} />
          </g>
        ))}
        <Callout n={1} x={W - 60} y={170} />
      </Card>
      <Line x={32} y={244} text={t('kiosk.assistCorrectHint', 'Fix a typo in the fields and verify again, or end the assistance.')} size={11} color={K.muted} max={100} />
      <Btn x={32} y={258} w={220} label={t('kiosk.assistVerifyBtn', 'Verify and continue')} tone="staff" callout={2} />
      <Btn x={266} y={258} w={200} label={t('kiosk.assistEndBtn', 'End assistance')} tone="danger" callout={3} />
    </KioskFrame>
  );
}
  ...
  return (
    <KioskFrame step={2} help={false} label={t('kiosk.assistNameTitle', 'Confirm the guest’s name')}>
      ...
      <Card x={32} y={116} w={280} h={70}>
        <Line x={48} y={138} text={t('kiosk.assistNameLicense', 'Name on the licence')} size={10} weight={700} color={K.muted} />
        <Line x={48} y={166} text="ROBERTO DIAZ MORALES" size={14} weight={600} color={K.ink} />
      </Card>
      <Card x={328} y={116} w={280} h={70}>
        <Line x={344} y={138} text={t('kiosk.assistNameReservation', 'Name on the reservation')} size={10} weight={700} color={K.muted} />
        <Line x={344} y={166} text="ROBERTO DIAZ" size={14} weight={600} color={K.ink} />
      </Card>
      <Callout n={1} x={320} y={116} />
      <Line x={32} y={214} text={t('kiosk.assistNameHint', 'You are certifying that you checked the physical licence and it belongs to this guest.')} size={11} color={K.muted} max={100} />
      <Btn x={32} y={236} w={330} label={t('kiosk.assistNameConfirmBtn', 'I verified this licence belongs to the guest')} tone="staff" callout={2} />
      <Btn x={376} y={236} w={160} label={t('kiosk.assistEndBtn', 'End assistance')} tone="danger" />
    </KioskFrame>
  );
}
