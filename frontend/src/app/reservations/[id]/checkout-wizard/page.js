...
export default function RetiredCheckoutWizardRedirect() {
  const { id } = useParams();
  const router = useRouter();
  useEffect(() => {
    router.replace(`/reservations/${id}/checkout-wizard-v2`);
  }, [id, router]);
  return <div style={{ padding: 24, color: '#6B7280' }}>Redirecting to check-out…</div>;
}
