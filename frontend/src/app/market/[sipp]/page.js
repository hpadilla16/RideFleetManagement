...
function ChartCard({ history, summaryRow, days, onDaysChange, loading }) {
  ...
  function vendorAlignedSeries(points) {
    const byDate = new Map((points || []).map((p) => [String(p.date).slice(0, 10), Number(p.price)]));
  ...
  const p25 = series.map((row) => Number(row.p25));
  const p75 = series.map((row) => Number(row.p75));
  const median = series.map((row) => Number(row.median));
  const cheap = series.map((row) => Number(row.min));
