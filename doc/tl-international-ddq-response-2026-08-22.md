...
| User authentication, privileged access, MFA | TOTP two-factor authentication enforced for all staff; see 3.3. |
...
| Credential, secret, certificate and key management | Secrets are held as environment variables on the host, and no secrets are committed to source control (verified continuously by automated secret scanning over full git history). **The field-encryption data key is managed by AWS KMS (envelope encryption): the key is stored only in KMS-wrapped form and is unwrapped into memory at process start via KMS — it is never stored as plaintext on the host** — and the ciphertext format carries a key version to support rotation. TLS certificates are issued by Let's Encrypt with automated renewal. |
| Security monitoring and alerting | Application error monitoring with alerting. The administrative/security audit trail is centralised into a dedicated log-management / SIEM platform (Axiom, US data region) for retention, search and alerting. Network intrusion detection (IDS) is not in place. |
