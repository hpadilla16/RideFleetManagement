...
| Vulnerability scanning and remediation | **`GAP` — none.** Continuous integration runs an extensive functional and authorisation-regression test suite but performs no dependency vulnerability scanning, static analysis, secret scanning or container image scanning. Remediation: dependency auditing and automated dependency updates. |
| Penetration / security testing | **`GAP` — no independent test has been performed.** |
