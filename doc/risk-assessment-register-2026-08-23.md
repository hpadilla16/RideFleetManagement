...
| R4 | Personal data at rest exposed if the DB/storage layer is compromised | 1 | 3 | 3 Low | Platform encryption at rest; **field-level PII encryption (planned)** | Eng | Treatment planned |
| R5 | Static field-encryption key exposure (no rotation/KMS) | 2 | 2 | 4 Med | Key held off-repo; **KMS/vault + rotation (planned)** | Eng | Treatment planned |
...
| R8 | Undetected security event (no SIEM) | 2 | 2 | 4 Med | Audit trail + error monitoring/alerting; **SIEM / log retention maturity (planned)** | Eng | Treatment planned |
...
| R12 | Availability / outage | 2 | 2 | 4 Med | Zero-downtime deploys; health gating; managed platform; **BC/DR plan (planned)** | Eng | Partial |
...
- **Already mitigated (residual Low):** R1, R2, R3, R9, R10, R11.
- **Treatment planned (time-bound in the roadmap):** R4, R5, R6, R7, R8, R12, R13, R14, R15.
