...
| **F1** | Schema + `purpose PARTNER` + `/api/partnerships` + admin UI + `GET /partners/:slug` + filtros `partnerId:null` — **dark** (`partnershipsEnabled=false`) | **CONSTRUIDO 2026-09-05** en worktree `~/Code/rfm-wt-partnerships` (rama `feat/partnerships-module-f1` sobre `origin/main`); E2E en browser local OK (crear → tarifa → precios → términos saneados → publicar → QR → switch inventario/preferencia de tipo → servicio solo-partner oculto del catálogo; AGENT 403). Innovation ✓ build (4 MC: sanitizador de texto decodifica, relink reactiva la tarifa, MI/PricingRule/long-term excluyen PARTNER, descuento excluyente con tarifa) · GD ✓ build (5 MC: colisión con `button` global en tema claro, patrón `.modal-backdrop`, QR bloqueado legible, strings i18n) · QA FIX-FIRST→fixes · **PR #115 mergeado y DEPLOYADO 2026-09-05 (sha `24142b41`)**: migraciones aplicadas por startup-migrate y registradas por 5432, 3 contenedores healthy, /api/health 200. Dark: ningún tenant con `partnershipsEnabled` |
| **F2** | `rental-search`/`checkout` con partner + `resolveForRental(rateId)` + storefront `/partners/[slug]` + subdominio + código de programa en staff | **CONSTRUIDO 2026-09-05** — RFM en `~/Code/rfm-wt-partnerships` (rama `feat/partnerships-f2` sobre `24142b41`): `partner-booking.js` (resolvePartnerContext fail-closed: ACTIVE+ventana+módulo+sede+tarifa-o-descuento; `partnerCheckoutRequirements` exige tipo ofrecido + disclosure para PREFERRED_TYPE; servicios obligatorios), `searchRental({partnerSlug})` (cache key `partner=<id>:<updatedAt>`, RATE→`resolveForRental({rateId})`, DISCOUNT→`applyDiscount` sobre el base online, revenue pricing bypass, sin depósito si no hay pago online), `createPublicBooking` re-resuelve el partner en servidor (nunca confía en el cliente), `PTR-` + `sourceRef PARTNER:<slug>:<hex>` + `bookingChannel PARTNER` + sellos de disclosure; landing en modo DESCUENTO publica "desde" = online × (1−%) . Storefront (`triangle-dealers-site`, sin commit aún): `partners.<host>/<slug>` se REESCRIBE (no redirige) a `/<locale>/partners/<slug>` con locale por cookie `NEXT_LOCALE` → Accept-Language → es, y `/es/partners/<slug>` vuelve al corto (URL del QR intacta); landing co-brand + PartnerPreferenceForm (aseguradoras) + PartnerPill en rent/vehículo/checkout; precio del programa con centavos ($35→$29.75, total $119 = lo que cobra checkout); vehículo/lista/checkout/confirmación con líneas del programa; sin número ni pago para PREFERRED_TYPE. **E2E local OK** (aseguradora: landing → disclosure → checkout sin pago → `PTR-…` CONFIRMED con sellos; descuento: lista → vehículo → checkout $119+tax). Tests: test:partnerships 20/20 · test:rates 20/20 (DB) · vitest 21/21 · tsc limpio. Reviews: Innovation OK (2 MC: programa no disponible a mitad del funnel → landing "no disponible", nunca precio público; portal/email sin cifras para reservas confirm-at-pickup) · GD OK (5 MC: `.prose-terms`, sin upsell de protección en checkout de aseguradora, etiqueta Total en confirmación, tap 44px, copy) · QA FIX-FIRST → M1 `fee-channel-filter` trataba PARTNER como STAFF (la primera relectura de pricing borraba las fees obligatorias del website cotizadas en el checkout) → PARTNER es canal online + test · M2 el storefront no enseñaba el catálogo del programa que RFM fuerza (servicios obligatorios) → el payload de `vehicle-classes` lleva `additionalServices` y el checkout suma picks ∪ obligatorios (verificado: $119 + $25 + tax = $157.69). Gates restantes: QA re-verify → **Hector revisa los diffs money-adjacent** → PR/CI → deploy RFM (Hector) → Vercel: dominio `partners.rentandgopr.com` + `ALLOWED_HOSTS` + CNAME `partners` → `cname.vercel-dns.com` |
