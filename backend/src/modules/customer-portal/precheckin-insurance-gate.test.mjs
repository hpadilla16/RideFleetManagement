...
function stubPathToDeclineBranch() {
  ...
  realFns['reservation.update'] = prisma.reservation.update;
  realFns['auditLog.create'] = prisma.auditLog.create;
  prisma.reservationPricingSnapshot.findUnique = async () => null;
  prisma.reservationCharge.findMany = async () => [];
  prisma.reservation.update = async (op) => { writes.push(['reservation.update', op]); return {}; };
  prisma.auditLog.create = async (op) => { writes.push(['auditLog.create', op]); return {}; };
  const realDiscount = settingsService.getPrecheckinDiscount;
  settingsService.getPrecheckinDiscount = async () => null;

  // THE CHARGE SHEET MOVED INTO A TRANSACTION (merge of 2026-08-28).
  //
  // The decline branch these two cases exercise used to be inline in
  // customer-portal.routes.js, running on the singleton this suite stubs. It
  // now lives in applyPrecheckinCharges (precheckin-charges.js), inside
  // `client.$transaction(async (tx) => ...)`. Prisma hands an interactive
  // transaction a NEW client, so the stubs above would be invisible to it and
  // the callback would try to reach a real Postgres.
  //
  // Run the callback against the stubbed singleton. What these cases assert is
  // WHICH write the decline branch issues -- the fenced updateMany versus the
  // bare update -- and that is unchanged by where the transaction boundary is.
  // Transaction semantics proper (atomicity, the advisory lock, the double-tap
  // 409) are covered against a real cluster in precheckin-charges.embedded.test.mjs.
  const realTransaction = prisma.$transaction;
  const realQueryRaw = prisma.$queryRaw;
  prisma.$transaction = async (fn) => fn(prisma);
  // The per-reservation advisory lock: report it as GRANTED, which is the
  // single-request case. Contention is the embedded suite's subject.
  prisma.$queryRaw = async () => [{ locked: true }];

  return () => {
    settingsService.getPrecheckinDiscount = realDiscount;
