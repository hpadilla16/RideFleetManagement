/**
 * Mex (TSD RezCentral) integration — unit tests.
 *
 ...
 * doc/mex-integration-plan-2026-07-13.md.
 *
 ...
 *  10. sourceSpec wired into the SHARED promoter (MEX- prefix, pay-at-destination).
...
test('constants: source identity is MEX / FRANCHISE_MEX / MEX-', () => {
  assert.equal(SOURCE_SYSTEM, 'MEX');
  assert.equal(BOOKING_CHANNEL, 'FRANCHISE_MEX');
  assert.equal(RESERVATION_PREFIX, 'MEX-');
  assert.equal(QUEUE_NAME, 'mex.sync');
...
test('parseTMSummary: a MISSING required column THROWS instead of importing a shifted mapping', () => {
  ...
  assert.throws(
    ...
    (err) => err instanceof MexLayoutError && /totalbill/i.test(err.message)
  );
});
...
async function primeSession(tenantId, fetchStub) {
  ...
  const { login } = await import('./mex.service.js');
...
test('mapRowToExternalReservation: the real row maps Total Bill → totalAmount, ACRISS inline, POA', () => {
  ...
  assert.equal(mapped.subBrand, 'MEX');
...
// Mex row to the REAL shared matcher, so the one combination that matters —
...
// MARIA GARCIA is the stranger; MARIO GARCIA is the incoming Mex booking.
...
/** Build a staged Mex row (as the worker upserts it) for one T&M row. */
...
test('MC1: a no-email Mex row does NOT match a stranger in the placeholder-phone pool', async () => {
...
test('MC1: an email row matches by email — the normal Mex happy path is intact', async () => {
  const pool = [
    ...PLACEHOLDER_POOL,
...
test('sourceSpec: the shared promoter writes MEX-<ref> + FRANCHISE_MEX + estimatedTotal only', async () => {
  ...
  assert.equal(r.reservationNumber, 'MEX-A1TL012880');
  assert.equal(r.bookingChannel, 'FRANCHISE_MEX');
  assert.equal(r.estimatedTotal, 135.85);
  assert.equal(r.isPrepaid, false);
  assert.match(r.notes, /Imported from Mex — A1TL012880 \(pay-at-destination\)/);
