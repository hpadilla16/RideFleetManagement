/**
 * Mex (TSD RezCentral) integration — shared constants.
 *
 ...
 *
 * WHY this source is different from its siblings: Mex does NOT have its own
 ...
 *
 * MONEY: Mex is 100% Pay on Arrival (confirmed with Mex by Hector,
...
export const ROOT_PATH = process.env.MEX_ROOT_PATH || '/WebRezClient/';
...
export const LOGIN_PATH = process.env.MEX_LOGIN_PATH || '/WebRezClient/WebLogin.aspx?id=vufnda45gf12r2ifchif';
...
// tracks authentication separately (see hasSessionCookie in mex.service.js).
...
// ---------------------------------------------------------------------------
export const LOGIN_USER_FIELD = process.env.MEX_LOGIN_USER_FIELD || 'txtUserID';
export const LOGIN_PASS_FIELD = process.env.MEX_LOGIN_PASS_FIELD || 'txtPwd';
export const LOGIN_IMAGE_BUTTON = process.env.MEX_LOGIN_IMAGE_BUTTON || 'imgLogin';
// Any coordinate inside the image works; WebForms only checks that the pair exists.
export const LOGIN_IMAGE_X = process.env.MEX_LOGIN_IMAGE_X || '10';
export const LOGIN_IMAGE_Y = process.env.MEX_LOGIN_IMAGE_Y || '10';
...
export const FIELD = Object.freeze({
  TSD_NUMBER: process.env.MEX_FIELD_TSD_NUMBER || 'lstTSDNumber',
  BRANCH: process.env.MEX_FIELD_BRANCH || 'lstBranch',
  CLASS: process.env.MEX_FIELD_CLASS || 'lstClass',
  RATE_CODE: process.env.MEX_FIELD_RATE_CODE || 'lstRateCode',
  TRADING_PARTNER: process.env.MEX_FIELD_TRADING_PARTNER || 'lstTradingPartner',
  SYSTEM: process.env.MEX_FIELD_SYSTEM || 'lstSystem',
  ...
  RUN_BY: process.env.MEX_FIELD_RUN_BY || 'lstRunBy',
  // The month the calendars DISPLAY. Both autopostback; they do not themselves
  // set the range. 73 options, labelled "Jul 2026".
  FROM_MONTH: process.env.MEX_FIELD_FROM_MONTH || 'lstFromMonth',
  TO_MONTH: process.env.MEX_FIELD_TO_MONTH || 'lstToMonth',
  // <input type="submit" value="Submit"> — runs the report, renders the grid.
  SUBMIT: process.env.MEX_FIELD_SUBMIT || 'Button1',
  ...
  SUMMARY: process.env.MEX_FIELD_SUMMARY || 'chkSummary',
});
...
export const CAL_TARGET = Object.freeze({
  FROM: process.env.MEX_CAL_FROM || 'CalFrom',
  TO: process.env.MEX_CAL_TO || 'CalTo',
});
...
// diagnostics in mex.service.js, which still emit both spans so the claim
...
export const RUN_BY_VALUE = process.env.MEX_RUN_BY || RUN_BY.DATE_OUT;
...
export const GRID_ID_SUFFIX = process.env.MEX_GRID_ID_SUFFIX || 'dgRates';
...
//   → toCalendarDaySerial() below; the posting lives in mex.service.js.
//
...
//   two postbacks are now SEQUENCED (see applyRange in mex.service.js).
//
// MODES (MEX_RANGE_MODE):
//
...
/**
 * Normalize an MEX_RANGE_MODE value. Unknown → the default (an unknown mode
...
export const RANGE_MODE = normalizeRangeMode(process.env.MEX_RANGE_MODE);
...
export const RANGE_SHORTCUT_FIELD = process.env.MEX_RANGE_SHORTCUT_FIELD || 'cmdMonthTD';
export const RANGE_SHORTCUT_VALUE = process.env.MEX_RANGE_SHORTCUT_VALUE || 'Current MTD';
// The Submit button's rendered value.
export const SUBMIT_VALUE = process.env.MEX_SUBMIT_VALUE || 'Submit';
...
// Missing any → MexLayoutError rather than a silently shifted import.
...
// ---------------------------------------------------------------------------
// Timezone. The active account is Mex ORLANDO (61302.MCO) → US Eastern.
...
// ---------------------------------------------------------------------------
export const TIME_ZONE = process.env.MEX_TIME_ZONE || 'America/New_York';

// ---------------------------------------------------------------------------
// Near-term date window (mirror NU/Economy/Flexways → MEX_* env names). The
// worker queries pickups (`Date Out`) in [today - LOOKBACK, today + LOOKAHEAD].
// Per-config lookbackDays/lookaheadDays on MexLocationConfig override these
...
export const DATE_WINDOW_LOOKBACK_DAYS = Number(
  process.env.MEX_DATE_WINDOW_LOOKBACK_DAYS ?? 2
);
export const DATE_WINDOW_LOOKAHEAD_DAYS = Number(
  process.env.MEX_DATE_WINDOW_LOOKAHEAD_DAYS ?? 30
);
