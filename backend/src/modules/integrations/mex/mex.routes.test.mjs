/**
 * Mex routes — MODULE tests, mirroring flexways.routes.test.mjs: lock the
 ...
 *   below: the real frontend builder (frontend/src/lib/mex-location-contract.js)
 ...
 *   this goes red.
 *
 * Mex KEY DIFFERENCE vs Flexways: a config row's identity is the PAIR
...
const {
  buildMexLocationBody,
  MEX_LOCATION_BODY_FIELDS,
} = await import('../../../../../frontend/src/lib/mex-location-contract.js');

// ---------------------------------------------------------------------------
// Endpoint inventory + guard
// ---------------------------------------------------------------------------

test('mexRouter mounts the full Fase 5 endpoint inventory (Flexways shape)', () => {
  assert.ok(mexRouter);
  const paths = mexRouter.stack.filter((l) => l.route).map((l) => l.route.path);
  const expected = [
    '/status',
    '/enabled',
    '/credentials',
...
test('mexRouter has an admin auth guard as its FIRST middleware layer', () => {
  const firstRouteIdx = mexRouter.stack.findIndex((l) => l.route);
  const guardLayers = mexRouter.stack.slice(0, firstRouteIdx).filter((l) => !l.route);
  assert.ok(guardLayers.length >= 1, 'expected auth/role guard middleware before routes');
});

test('mexRouter exposes mutations as POST/PUT/DELETE and reads as GET', () => {
  const byPath = new Map();
  for (const l of mexRouter.stack) {
    if (!l.route) continue;
...
test('location config body: the PANEL\'s builder feeds the ROUTES\' reader (beta.301)', () => {
  // Exactly what MexIntegrationPanel's "Add location" form builds.
  const panelBody = buildMexLocationBody({
    tsdNumber: '61302',
    branch: 'MCO',
...
test('panel PATCH bodies (partial edits) round-trip as "leave the rest alone"', () => {
  ...
  const read = readLocationBody(buildMexLocationBody({ locationId: 'loc2' }));
  ...
  const cleared = readLocationBody(buildMexLocationBody({ lookbackDays: null }));
...
test('branch is upper-cased on BOTH sides — a lowercase entry still matches the portal Loc', () => {
  ...
  // match a fetched "MCO".
  assert.equal(buildMexLocationBody({ branch: 'mco' }).branch, 'MCO');
