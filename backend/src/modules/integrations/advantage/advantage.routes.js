/**
 * Admin routes for the Advantage (TSD RezCentral) franchise integration (Fase 5).
 * A sibling of flexways.routes.js — same auth guard, same tenant-scoping, same
 * feature-flag posture — adapted for Advantage's KEY DIFFERENCE: a config row is
 * identified by TWO columns, (tsdNumber, branch), not Flexways' single idSede.
 * TSD's report screen keys inventory by `lstTSDNumber` (61302 = Advantage
 ...
 *
 * Mounted at `/api/admin/integrations/advantage` (tenantRateLimit + router). Every
 ...
 *
 * Master enable/disable lives in Tenant.integrationConfig.advantage.enabled (Json,
 * additive). Credentials are ONE {username,password} blob per tenant in
 * IntegrationCredential (unique (tenantId, sourceSystem='ADVANTAGE')), encrypted
 ...
 * (frontend/src/lib/advantage-location-contract.js). advantage.routes.test.mjs
 * feeds the panel's builder output straight into this reader, so the two can never
 * drift silently again.
 *
 * MONEY: none. Advantage is 100% Pay on Arrival (confirmed 2026-07-14) — the
 ...
 *   POST  /run-now                                 → enqueue a one-off advantage.sync job
 *   GET   /runs?limit=                             → recent ExternalSyncRun rows
 *   GET   /locations                               → AdvantageLocationConfig rows (+ Ride location)
 ...
 *   GET   /pending-imports                         → MANUAL_REVIEW / PENDING tray (source ADVANTAGE)
 *   POST  /pending-imports/:id/promote             → manual promote (source ADVANTAGE)
 *   POST  /pending-imports/:id/reject              → reject
 *
 * Feature-flag posture (mirror TL/Economy/NU/Flexways): routes ALWAYS exist and
 * config is ALWAYS editable. ADVANTAGE_INTEGRATION_ENABLED gates only the
...
import {
  ...
  forceRelogin,
  SOURCE_SYSTEM,
} from './advantage.service.js';
import { enqueueOneOffSync, promoteWithMappings } from './advantage.worker.js';
import { effectiveWindowDays, TIME_ZONE } from './advantage.constants.js';
import { integrationEnabled } from './advantage.scheduler.js';

export const advantageRouter = Router();

// Same authorization as TL/Economy/NU/Flexways: SUPER_ADMIN + ADMIN, ADMIN hard-scoped.
advantageRouter.use(requireAuth, requireRole('SUPER_ADMIN', 'ADMIN'));
...
// Master enable flag lives in Tenant.integrationConfig.advantage.enabled (JSON,
...
async function readMasterEnabled(tenantId) {
  ...
  return !!(cfg && typeof cfg === 'object' && cfg.advantage && cfg.advantage.enabled === true);
}
...
async function writeMasterEnabled(tenantId, enabled) {
  ...
  const next = {
    ...base,
    advantage: {
      ...(base.advantage && typeof base.advantage === 'object' ? base.advantage : {}),
      enabled: !!enabled,
    },
  };
...
function estimateNextRunAt(lastRun) {
  ...
  const minutes = Number(process.env.ADVANTAGE_SYNC_INTERVAL_MINUTES || 15);
...
// builds its body through buildAdvantageLocationBody() (frontend lib). The test
...
// ---------------------------------------------------------------------------
// GET /status — health summary for the Advantage panel (mockup AD-1 / AD-3)
// ---------------------------------------------------------------------------

advantageRouter.get('/status', asyncHandler(async (req, res) => {
...
advantageRouter.put('/enabled', asyncHandler(async (req, res) => {
  ...
  logger.info('[advantage-routes] master enable toggled', { tenantId, enabled: next, userId: req.user?.id });
...
advantageRouter.post('/credentials', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  ...
  logger.info('[advantage-routes] credentials set', { tenantId, credentialId: row.id, userId: req.user?.id });
...
advantageRouter.post('/test-auth', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
...
advantageRouter.post('/force-relogin', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  try {
    const result = await forceRelogin(tenantId);
    logger.info('[advantage-routes] force re-login', { tenantId, userId: req.user?.id });
...
advantageRouter.post('/run-now', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  ...
  logger.info('[advantage-routes] manual sync enqueued', { tenantId, jobId, userId: req.user?.id });
...
advantageRouter.get('/runs', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantIdOrNull(req);
...
// ---------------------------------------------------------------------------
// AdvantageLocationConfig CRUD — the per-(TSD account, branch) mapping.
...
advantageRouter.get('/locations', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantIdOrNull(req);
...
advantageRouter.post('/locations', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  ...
  const existing = await prisma.advantageLocationConfig.findUnique({
    where: {
  ...
  const row = await prisma.advantageLocationConfig.create({
    data: {
      tenantId,
  ...
  logger.info('[advantage-routes] location mapping created', {
...
advantageRouter.put('/locations/:id', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  const { id } = req.params;

  const owned = await prisma.advantageLocationConfig.findFirst({
    where: { id, tenantId },
  ...
  if (body.tsdNumber !== undefined || body.branch !== undefined) {
    ...
    const clash = await prisma.advantageLocationConfig.findUnique({
  ...
  const row = await prisma.advantageLocationConfig.update({ where: { id }, data });
  res.json({ ok: true, row });
}));

// POST /locations/:id/toggle — flip enabled on the mapping.
advantageRouter.post('/locations/:id/toggle', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  const { id } = req.params;
  if (typeof req.body?.enabled !== 'boolean') return send400(res, 'enabled (boolean) is required');
  const nextEnabled = !!req.body.enabled;

  const updated = await prisma.advantageLocationConfig.updateMany({
    where: { id, tenantId },
...
// DELETE /locations/:id
advantageRouter.delete('/locations/:id', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  const { id } = req.params;
  const deleted = await prisma.advantageLocationConfig.deleteMany({ where: { id, tenantId } });
...
advantageRouter.get('/pending-imports', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantIdOrNull(req);
  const where = {
    sourceSystem: SOURCE_SYSTEM,
...
// ---------------------------------------------------------------------------
// POST /pending-imports/:id/promote — manual promotion (source ADVANTAGE).
//
...
// 400, same guard as NU/Flexways. ADVANTAGE TZ (America/New_York).
// ---------------------------------------------------------------------------

advantageRouter.post('/pending-imports/:id/promote', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  const { id } = req.params;
...
advantageRouter.post('/pending-imports/:id/reject', asyncHandler(async (req, res) => {
  const tenantId = resolveTenantId(req);
  const { id } = req.params;
...
export default advantageRouter;
