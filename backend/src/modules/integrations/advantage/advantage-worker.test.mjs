/**
 * Advantage sync worker — HANDLER-level guards (Innovation gate, 2026-07-16).
 *
...
const svc = await import('./advantage.service.js');
const { advantageSyncHandler } = await import('./advantage.worker.js');
const { toMonthOptionLabel, MENU_PATH_EMAIL } = await import('./advantage.constants.js');
...
async function runHandler(portalPlan = {}, prismaPlan = {}) {
  ...
  try {
    const result = await advantageSyncHandler({ data: { tenantId: 't1', triggeredBy: 'test' } });
