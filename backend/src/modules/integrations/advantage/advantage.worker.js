/**
 * Advantage (TSD RezCentral) sync worker — one BullMQ job per tenant that pulls
 ...
 *   - createPromoter(advantageSourceSpec)  → promote / link (MONEY: estimatedTotal only)
 ...
 * (tsdNumber, branch) pairs → N Ride locations via AdvantageLocationConfig. Each
 ...
 * Advantage is 100% Pay on Arrival → isPrepaid is always false.
 *
...
// ---------------------------------------------------------------------------
// The sourceSpec for the shared promoter. Advantage is 100% Pay on Arrival
// (confirmed with Advantage 2026-07-14), so every promoted row carries the
...
// ---------------------------------------------------------------------------
const advantageSourceSpec = Object.freeze({
...
const { promoteAutomatically, promoteWithMappings } = createPromoter(advantageSourceSpec);
export { promoteAutomatically, promoteWithMappings };

function autoCreateCustomersEnabled() {
  return autoCreateEnabledFromEnv('ADVANTAGE_AUTO_CREATE_CUSTOMERS');
}
...
/**
 * NO-EMAIL HANDLING — Hector's decision (A), 2026-07-14. ~50% of Advantage rows
 ...
 * row's phone null is the belt to its braces: Advantage never had a phone to
 * stage in the first place.
 *
 * This is done HERE, in Advantage's mapping. The shared helper is NOT changed, so
...
export function mapRowToExternalReservation(row) {
  ...
  return {
    ...
    currency: 'USD',
    // Advantage is 100% Pay on Arrival (confirmed 2026-07-14). If that ever
 ...
 */
export async function advantageSyncHandler(job) {
  const { tenantId, triggeredBy = 'schedule' } = job?.data || {};
  if (!tenantId) throw new Error('advantage.sync: job.data.tenantId is required');

  const userAgent = pickUserAgent();
  logger.info('[advantage-sync] starting run', { tenantId, triggeredBy });
  ...
  try {
    if (configs.length === 0) {
      logger.info('[advantage-sync] no enabled AdvantageLocationConfig for tenant — nothing to sync', { tenantId });
    }
    ...
    for (const config of configs) {
      ...
      try {
        ...
        if (err instanceof AdvantageAuthExpiredError) throw err; // fail the whole run
        if (err instanceof AdvantageLayoutError) throw err;      // ATTENTION, not a silent skip
        errorsCount++;
      ...
      try {
        ...
        if (err instanceof AdvantageAuthExpiredError) throw err;
        ...
        // row as email-less.
        if (err instanceof AdvantageLayoutError) throw err;
        errorsCount++;
        errorSamples.push(`${config.tsdNumber}.${config.branch}: email report failed (rows imported WITHOUT email enrichment): ${err.message}`);
        logger.warn('[advantage-sync] email report failed; continuing without email enrichment', {
      ...
      logger.info('[advantage-sync] config feed fetched', {
        tenantId,
        runId: runRow.id,
      ...
      if (d.rangeCoverage?.covers === false) {
        rangeGapNotes.push(
          ...
          + 'of the window are NOT fetched (set ADVANTAGE_RANGE_MODE=explicit for full coverage)'
        );
      }
      if (d.unparsedRows) {
        rangeGapNotes.push(
      ...
      for (const row of rows) {
        ...
        try {
          ...
          if (ownerTenantId && ownerTenantId !== tenantId) {
            skippedCrossTenant++;
            logger.warn('[advantage-sync] cross-tenant ExternalReservation collision — row skipped', {
          ...
          if (alreadyPromoted && !promotable) {
            ...
            logger.warn('[advantage-sync] source says CANCELLED/NO-SHOW but the row is already promoted — live Reservation left untouched (manual review)', {
          ...
          if (decision.decision === 'MANUAL_REVIEW'
            ...
            try {
              ...
              const newCust = await maybeCreateCustomerFromSource(prisma, {
                ...
                logPrefix: '[advantage-sync]', sourceName: 'Advantage',
              });
              ...
              logger.warn('[advantage-sync] auto-create customer failed; MANUAL_REVIEW', {
    ...
    if (err instanceof AdvantageAuthExpiredError) {
      ...
      logger.warn('[advantage-sync] session expired', { tenantId, runId: runRow.id, message: err.message });
    ...
    } else if (err instanceof AdvantageLayoutError) {
      ...
      logger.error('[advantage-sync] DataGrid layout drift — refusing to import a shifted mapping', {
      ...
      finalStatus = 'FAILED';
      logger.error('[advantage-sync] fatal error', {
...
export async function enqueueOneOffSync(tenantId, triggeredBy = 'manual') {
  return enqueueJob(QUEUE_NAME, { tenantId, triggeredBy }, {
    jobId: `advantage-sync:${tenantId}:${Date.now()}`,
    priority: 5,
  });
}

/** Register the worker with BullMQ. Idempotent — call once at worker boot. */
export function registerAdvantageSyncWorker() {
  registerWorker(QUEUE_NAME, advantageSyncHandler, {
    concurrency: 1,
    priority: SCRAPER_PRIORITY,
  });
  logger.info('[advantage-sync] worker registered', { queue: QUEUE_NAME });
}
